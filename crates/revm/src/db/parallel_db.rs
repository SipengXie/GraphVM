...
impl<DB: DatabaseRef> DatabaseRef for ParallelDB<DB> {
    ...
    fn basic_ref(&self, address: Address) -> Result<Option<AccountInfo>, Self::Error> {
        let stats = self.get_current_stats();
        {
            ...
            if let Some(acc) = cache.accounts.get(&address) {
                ...
                stats.cache_time.fetch_add(cache_start.elapsed().as_nanos() as u64, Ordering::Relaxed);
                return Ok(acc.info());
            }
            stats.cache_time.fetch_add(cache_start.elapsed().as_nanos() as u64, Ordering::Relaxed);
        }
    ...
    fn storage_ref(&self, address: Address, index: U256) -> Result<U256, Self::Error> {
        let stats = self.get_current_stats();
        {
            ...
            if let Some(acc) = cache.accounts.get(&address) {
                if let Some(value) = acc.storage.get(&index) {
                    ...
                    stats.cache_time.fetch_add(cache_start.elapsed().as_nanos() as u64, Ordering::Relaxed);
                    return Ok(*value);
                } else if matches!(acc.account_state, AccountState::StorageCleared | AccountState::NotExisting) {
                    stats.cache_hits.fetch_add(1, Ordering::Relaxed);
                    stats.cache_time.fetch_add(cache_start.elapsed().as_nanos() as u64, Ordering::Relaxed);
                    return Ok(U256::ZERO);
                }
        ...
        if !matches!(acc.account_state, AccountState::StorageCleared | AccountState::NotExisting) {
