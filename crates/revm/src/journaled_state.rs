...
impl ReadWriteSet {
    ...
    pub fn add_write(&mut self, address: Address, access_type: AccessType) {
        self.write_set
            .entry(address)
            .or_insert_with(HashSet::default)
            .insert(access_type);
    ...
    /// Check if the current ReadWriteSet has a conflict with another ReadWriteSet
    /// A conflict is considered to exist if any element in self's read_set is present in other's write_set
...
impl JournaledState {
    ...
    pub fn clear(&mut self) {
        let spec = self.spec;
        let read_write_set = self.read_write_set();
    ...
    pub fn sload<DB: Database>(
        ...
        if debug_list.contains(&address) {
            println!("sload: {:?} key {:?} value {:?}", &address, key, value);
        }
