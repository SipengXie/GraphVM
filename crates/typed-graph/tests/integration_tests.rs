...
use revm_primitives::{
    keccak256, uint, AccountInfo, AccountStatus, Address, Bytecode, Bytes, Env, SpecId, TxKind, U256,
};
...
pub fn execute_case(code: Bytes, case_name: &str, config: ExecutionConfig) -> std::time::Duration {
    ...
    let execution_time =
        typed_graph_execute(logs, config.clone(), &env, &first_call);
...
fn typed_graph_execute(
    ...
    println!("Converting {} SSA log entries to TypedGraph...", entries.len());
