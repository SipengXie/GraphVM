// use revm_primitives::{
//     uint, TxKind, U256, AccountInfo,
//     Env, Bytecode,
//     LatestSpec, SpecId,
// };

// use revm_primitives::db::{CacheDB, EmptyDB};
...
//             let logger = evm.take_ssa_logger().unwrap();
//             // eprintln!("{:?}",logger.get_first_reads());
//             let logs = logger.get_logs().to_vec();
...
//             tracer.record_original(entry.lsn, entry.outputs.clone());
//         }
//         Some(tracer)
//     } else {
//         None
...
//     let mut executor = SSAExecutor::<_, LatestSpec>::new(graph, db, env, config.thread_number).with_mode(config.mode).with_tracer(tracer);
...
//             mode: ExecutionMode::Partial(456),
...
//         let serial_partial_config = ExecutionConfig {
//             mode: ExecutionMode::Partial(186),
//             test_mode: TestMode::SerialGraph,
//             collect_metrics: true,
//             pre_deployed_contract: vec![],
//             pre_determined_slots: vec![],
...
//         let serial_partial_config = ExecutionConfig {
//             mode: ExecutionMode::Partial(343),
//             test_mode: TestMode::SerialGraph,
//             collect_metrics: true,
//             pre_deployed_contract: vec![],
//             pre_determined_slots: vec![
