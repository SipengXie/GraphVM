...
#[inline(always)]
pub fn execute_jump<DB: DatabaseRef + Send + Sync>(
...
#[inline(always)]
pub fn execute_jumpi<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_ret<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
    instruction_result: SSAInstructionResult,
) -> Result<()> {
    ...
    let new_size = calc_mem_size(offset, length);
...
#[inline(always)]
pub fn execute_change_instruction_result<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    _graph: &SsaGraph,
    opcode: u8,
