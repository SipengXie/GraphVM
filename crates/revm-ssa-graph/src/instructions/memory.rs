...
#[inline(always)]
pub fn calc_mem_size(offset: usize, size: usize) -> usize {
...
#[inline(always)]
pub fn execute_mload<DB: DatabaseRef + Send + Sync>(
    ...
    let new_size = calc_mem_size(offset, 32);
...
#[inline(always)]
pub fn execute_mstore<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_mstore8<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    // Calculate required memory size
    let new_size = calc_mem_size(offset, 1);
...
#[inline(always)]
pub fn execute_msize<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_mcopy<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    // Calculate required memory size
    let new_size = calc_mem_size(dst.max(src), len);
