...
#[inline(always)]
pub fn execute_deduct_caller<DB: DatabaseRef + Send + Sync>(
    ...
    let caller_info =
        get_storage_value!(graph, node.inputs[3], &key, |key| _context.get_state(key));
...
#[inline(always)]
pub fn execute_refund_gas<DB: DatabaseRef + Send + Sync, SPEC: Spec>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    if base_gas_remaining < dynamic_gas_cost {
        ...
        return Err(ExecutionError::ExecutionError(
            ExecutionError::GAS_INSUFFICIENT.to_string(),
        ));
    }
...
#[inline(always)]
pub fn execute_reward_beneficiary<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    let beneficiary_account_info =
        get_storage_value!(graph, node.inputs[1], &key, |key| _context.get_state(key));
...
#[inline(always)]
pub fn execute_call<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
    opcode: u8,
    ...
    let new_size_1 = if in_len == 0 {
        ...
        calc_mem_size(in_offset, in_len)
    };
    let new_size_2 = calc_mem_size(out_offset, out_len);
...
#[inline(always)]
pub fn execute_callcode<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
    opcode: u8,
    ...
    node.outputs[0] = SSAOutput::FrameInput(Box::new(ssa_call_input));
    let new_size_1 = calc_mem_size(in_offset, in_len);
    let new_size_2 = calc_mem_size(out_offset, out_len);
    let new_size = std::cmp::max(new_size_1, new_size_2);
    if new_size > _context.memory_size() {
        if node.outputs.len() < 2 {
...
#[inline(always)]
pub fn execute_delegatecall<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
    opcode: u8,
...
#[inline(always)]
pub fn execute_staticcall<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
    opcode: u8,
...
#[inline(always)]
pub fn execute_make_call_frame<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    let call_input = get_frame_input!(
        graph,
        node.inputs[0],
        _context.get_first_frame_input().unwrap()
    );
    ...
    let caller_info = get_storage_value!(graph, node.inputs[1], &caller_key, |key| _context
        .get_state(key));
    let target_info = get_storage_value!(graph, node.inputs[2], &target_key, |key| _context
        .get_state(key));
    let bytecode_info = get_storage_value!(graph, node.inputs[3], &bytecode_key, |key| _context
        .get_state(key));
    ...
    if _context.is_precompile(&bytecode_address) {
        // if is precompile ..
        let precompile =
            _context.call_precompile(&bytecode_address, &call_input.input, call_input.gas_limit);
...
#[inline(always)]
pub fn execute_call_return<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_insert_call_outcome<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_create<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    let new_size = calc_mem_size(as_usize_saturated!(code_offset), len);
...
#[inline(always)]
pub fn execute_make_create_frame<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    let create_input = match node.inputs[0] {
        SSAInput::FrameInput((lsn, index)) => {
            if lsn == 0 {
                &Box::new(_context.get_first_frame_input().unwrap())
            } else {
                let dep_node = graph.get_node(lsn)?;
                match &dep_node.outputs[index as usize] {
    ...
    let caller_info = get_storage_value!(graph, node.inputs[1], &caller_key, |key| _context
        .get_state(key));
    let created_info = get_storage_value!(graph, node.inputs[2], &created_key, |key| _context
        .get_state(key));
...
#[inline(always)]
pub fn execute_create_return<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_insert_create_outcome<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
