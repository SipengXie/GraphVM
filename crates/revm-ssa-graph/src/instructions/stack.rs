...
impl<'a, DB: DatabaseRef + Send + Sync, SPEC: Spec> ExecutionContext<'a, DB, SPEC> {
    ...
    #[inline]
    pub fn execute_push(&self, inputs: Vec<SSAOutput>, size: usize) -> Result<Vec<SSAOutput>> {
        if inputs.len() != 1 {
    ...
    #[inline]
    pub fn execute_pop(&self, inputs: Vec<SSAOutput>) -> Result<Vec<SSAOutput>> {
        if inputs.len() != 1 {
            return Err(ExecutionError::ExecutionError(
    ...
    #[inline]
    pub fn execute_dup(&self, inputs: Vec<SSAOutput>, position: usize) -> Result<Vec<SSAOutput>> {
        ...
        let value = match_ssa_output_stack_or_const!(&inputs[position - 1], format!("Position {}", position).as_str());
    ...
    #[inline]
    pub fn execute_swap(&self, inputs: Vec<SSAOutput>, position: usize) -> Result<Vec<SSAOutput>> {
        ...
        let top = match_ssa_output_stack_or_const!(&inputs[0], "First");
        let swap = match_ssa_output_stack_or_const!(&inputs[position], format!("Position {}", position + 1).as_str());
