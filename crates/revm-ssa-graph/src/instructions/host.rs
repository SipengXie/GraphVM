...
#[inline(always)]
pub fn execute_sload<DB: DatabaseRef + Send + Sync>(
    ...
    let value = get_storage_value!(graph, node.inputs[2], &value_key, |key| _context
        .get_state(key));
    let account_status =
        get_storage_value!(graph, node.inputs[3], &account_status_key, |key| _context
            .get_state(key));
...
#[inline(always)]
pub fn execute_sstore<DB: DatabaseRef + Send + Sync, SPEC: Spec>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    let origin_value = get_storage_value!(graph, node.inputs[3], &slot_key, |key| _context
        .get_state(key));
    let present_value = get_storage_value!(graph, node.inputs[4], &slot_key, |key| _context
        .get_state(key));
...
#[inline(always)]
pub fn execute_tstore<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    let _address = get_contract_env!(graph, node.inputs[0])
        .frame_input
        .target_address;
...
#[inline(always)]
pub fn execute_tload<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_balance<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    let account = get_storage_value!(graph, node.inputs[1], &account_key, |key| _context
        .get_state(key));
...
#[inline(always)]
pub fn execute_selfbalance<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_extcodesize<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_extcodehash<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_extcodecopy<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    let account_info = get_storage_value!(graph, node.inputs[4], &account_key, |key| _context
        .get_state(key));
    ...
    let new_size = calc_mem_size(mem_offset, len);
...
#[inline(always)]
pub fn execute_blockhash<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    let blockhash = _context.get_blockhash(number)?;
...
#[inline(always)]
pub fn execute_log<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
...
#[inline(always)]
pub fn execute_selfdestruct<DB: DatabaseRef + Send + Sync>(
    _context: &mut ExecutionContext<DB>,
    node: &mut SSALogEntry,
    graph: &SsaGraph,
) -> Result<()> {
    ...
    let address_info = get_storage_value!(graph, node.inputs[2], &address_info_key, |key| _context
        .get_state(key));
    let target_info = get_storage_value!(graph, node.inputs[3], &target_info_key, |key| _context
        .get_state(key));
    let address_status =
        get_storage_value!(graph, node.inputs[4], &address_status_key, |key| _context
            .get_state(key));
