...
use revm_primitives::{db::DatabaseRef, Spec, Env};
use revm_ssa::{
    logger::LsnType,
    SSACallInput,
    SSACreateInput,
    ...
    SPEC: Spec + Send + Sync,
{
    ...
    fn execute_node(node: &mut SSALogEntry, graph: & SsaGraph, context: &Arc<ExecutionContext<'a, DB, SPEC>>) -> Result<()> {
        ...
        match node.opcode {
            ...
            0xF3 => context.execute_ret(node, graph, 
                SSAInstructionResult::Ok),                       // RETURN
            ...
            0xFD => context.execute_ret(node, graph, 
                SSAInstructionResult::Revert),                   // REVERT
            ...
            _ => Err(ExecutionError::ExecutionError(
                format!("Unsupported opcode: 0x{:02x}", node.opcode)
            )),
        }
    }
