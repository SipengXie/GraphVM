...
pub fn address<H: Host + ?Sized>(interpreter: &mut Interpreter, _host: &mut H) {
    ...
    if let Some(logger) = interpreter.ssa_logger.as_mut() {
        logger.log_system_operation(
            ADDRESS,
            convert_contract_env_for_system(&interpreter.contract),
        );
    }
}
...
pub fn calldataload<H: Host + ?Sized>(interpreter: &mut Interpreter, _host: &mut H) {
    ...
    *offset_ptr = word.into();
    ...
    if let Some(logger) = interpreter.ssa_logger.as_mut() {
        logger.log_call_data_load(CALLDATALOAD, offset, interpreter.contract.input.clone());
    }
}
...
pub fn calldatacopy<H: Host + ?Sized>(interpreter: &mut Interpreter, _host: &mut H) {
    ...
    if len == 0 {
        if let Some(logger) = interpreter.ssa_logger.as_mut() {
            let mem_length = None;
            let lsn = logger.log_call_data_copy(
                CALLDATACOPY,
                memory_offset,
                data_offset,
                len,
...
pub fn returndatacopy<H: Host + ?Sized, SPEC: Spec>(interpreter: &mut Interpreter, _host: &mut H) {
    ...
    if len == 0 {
        if let Some(logger) = interpreter.ssa_logger.as_mut() {
            let mem_length = None;
            let lsn = logger.log_return_data_cpy_operation(
                RETURNDATACOPY,
                memory_offset,
                data_offset,
                len,
                interpreter.return_data_buffer.clone(),
                mem_length,
            );
            interpreter
                .shared_memory
